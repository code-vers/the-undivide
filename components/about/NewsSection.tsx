...
export default function NewsSection() {
  return (
    <section className="bg-[#f8f8f2] py-[120px] px-8 md:px-[80px]">
      <div className="mx-auto max-w-[1760px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[20px]">
          {newsItems.map((item, idx) => (
            <div key={idx} className="bg-white border border-[#dcfcc0] rounded-[18px] overflow-hidden group">
              <div className="h-[497px] overflow-hidden">
              ...
              <div className="p-[20px] space-y-3">
                ...
                <h3 className="text-[20px] font-bold text-[#1c3530] leading-snug line-clamp-2">
                  {item.title}
                </h3>
                <p className="text-[16px] text-[#3d5e57] line-clamp-2">
                  {item.description}
                </p>
                
                <div className="flex items-center gap-3 pt-4">
                  <span className="text-[13px] text-[#8fa39d]">{item.date}</span>
                  <span className="text-[13px] text-[#8fa39d]">• {item.source}</span>
                </div>

                <button className="flex items-center gap-2 pt-4 text-[14px] font-semibold text-[#2d584a] hover:opacity-80 transition-opacity">
                  Read More
        ...
        <div className="mt-16 flex justify-center">
          <Link href="/news">
            <button className="bg-[#2d584a] hover:bg-[#1c3530] text-white h-[64px] px-[32px] rounded-[12px] text-[16px] font-normal shadow-lg flex items-center gap-[12px] transition-colors">
              View All News
